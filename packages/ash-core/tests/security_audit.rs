...
fn test_rejects_empty_binding() {
    let nonce = "a".repeat(64);
    // Note: ash_derive_client_secret doesn't validate empty binding
    // but ash_build_proof does
    let secret = ash_derive_client_secret(&nonce, "ctx_test", "").unwrap();
    let result = ash_build_proof(&secret, "1700000000", "", &"b".repeat(64));
    assert!(result.is_err());
}

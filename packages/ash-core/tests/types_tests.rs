...
mod type_exports {
    ...
    fn test_ash_error_type() {
        let result: Result<String, AshError> = Err(AshError::new(AshErrorCode::MalformedRequest, "test"));
...
mod constant_exports {
    ...
    #[test]
    fn test_version_prefix_constant() {
        assert!(!ASH_VERSION_PREFIX.is_empty());
    }
...
mod error_types {
    ...
    fn test_error_is_displayable() {
        let err = AshError::new(AshErrorCode::MalformedRequest, "test error");
        let msg = format!("{}", err);
        assert!(!msg.is_empty());
    }
    ...
    fn test_error_variants() {
        let _ = AshError::new(AshErrorCode::MalformedRequest, "test");
    }
}
...
mod unified_proof_result_type {
    ...
    #[test]
    fn test_unified_result_implements_clone() {
        let result = UnifiedProofResult {
            proof: "a".repeat(64),
            scope_hash: String::new(),
            chain_hash: String::new(),

...
//! ## Overview
//!
//! ASH (Anti-tamper Security Hash) proofs are deterministic integrity tokens that:
...
pub const ASH_SDK_VERSION: &str = "2.3.4";
...
/// Maximum nonce length to prevent DoS via headers/storage.
...
///
/// # Deprecated
/// Prefer `generate_nonce()` which returns Result.
...
/// * `binding` - Canonical binding string (may contain `|` as part of the v2.3.2+ format)
///
/// # Errors
///
/// Returns error if:
...
/// This allows binding to contain `|` (as in the v2.3.2+ format `METHOD|PATH|QUERY`).
///
...
pub fn ash_derive_client_secret(nonce: &str, context_id: &str, binding: &str) -> Result<String, AshError> {
    ...
    // SEC-015 & BUG-001: Validate context_id doesn't contain delimiter
    // This prevents delimiter collision since binding may contain | (v2.3.2+ format)
    // Note: This check is now redundant due to charset validation above, but kept for clarity
    if context_id.contains('|') {
        return Err(AshError::new(
            AshErrorCode::MalformedRequest,
            "context_id must not contain '|' character (delimiter collision risk)",
        ));
    }
...
///
/// ```rust
/// use ash_core::{ash_verify_proof_with_freshness, ash_derive_client_secret, ash_build_proof, ash_hash_body};
///
/// let nonce = "0123456789abcdef0123456789abcdef";
/// let context_id = "ctx_abc123";
...
mod tests_proof {
    ...
    fn test_derive_client_secret_allows_delimiter_in_binding() {
        // BUG-001: Bindings with | are allowed (v2.3.2+ format uses METHOD|PATH|QUERY)
    ...
    // SEC-NONCE-001: Nonce length validation
    #[test]
    fn test_nonce_max_length() {
        let long_nonce = "0".repeat(129); // Over MAX_NONCE_LENGTH (128)
    ...
    fn test_nonce_at_max_length() {
        let max_nonce = "0".repeat(128); // Exactly MAX_NONCE_LENGTH
...
/// # Example
/// ```rust
/// use ash_core::ash_extract_scoped_fields_strict;
/// use serde_json::json;
///
...
fn ash_parse_all_array_indices(part: &str) -> ArrayNotation<'_> {
    ...
    // If there's trailing text after the last ], invalidate all indices
    // This handles cases like "items[0]extra" or "items[0][abc]"
    if !remaining.is_empty() {
        // Return with valid indices parsed so far (which may be empty)
        // but key is still the proper key part
...
pub fn ash_build_proof_scoped(
    ...
    let json_payload: Value = if payload.is_empty() || payload.trim().is_empty() {
        ...
        serde_json::from_str(payload)
            .map_err(|e| AshError::canonicalization_error(&format!("Invalid JSON: {}", e)))?
    };
...
///
/// ```rust
/// use ash_core::ash_hash_scoped_body_strict;
///
...
mod tests_bug046_047 {
    ...
    fn test_build_proof_scoped_rejects_empty_timestamp() {
        // BUG-046: Empty timestamp should be rejected
    ...
    fn test_build_proof_unified_rejects_empty_timestamp() {
        // BUG-047: Empty timestamp should be rejected

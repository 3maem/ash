...
fn vector_query_sorted() {
    let result = ash_core::ash_canonicalize_query("z=1&a=2&m=3").unwrap();
    assert_eq!(result, "a=2&m=3&z=1");
}

#[test]
fn vector_query_strip_leading_question_mark() {
    let result = ash_core::ash_canonicalize_query("?a=1&b=2").unwrap();
    assert_eq!(result, "a=1&b=2");
}

#[test]
fn vector_query_uppercase_hex() {
    let result = ash_core::ash_canonicalize_query("a=%2f&b=%2F").unwrap();
    assert_eq!(result, "a=%2F&b=%2F");
}

#[test]
fn vector_query_preserve_empty_values() {
    let result = ash_core::ash_canonicalize_query("a=&b=1").unwrap();
    assert_eq!(result, "a=&b=1");
}

#[test]
fn vector_query_duplicate_keys_sorted_by_value() {
    let result = ash_core::ash_canonicalize_query("a=z&a=a&a=m").unwrap();
...
fn vector_query_single_param() {
    let result = ash_core::ash_canonicalize_query("key=value").unwrap();
...
fn vector_urlencoded_plus_as_literal() {
    ...
    let result = ash_core::ash_canonicalize_urlencoded("a=hello+world").unwrap();
    assert_eq!(result, "a=hello%2Bworld");
}

#[test]
fn vector_urlencoded_uppercase_hex() {
    let result = ash_core::ash_canonicalize_urlencoded("a=hello%2fworld").unwrap();
    assert_eq!(result, "a=hello%2Fworld");
}

#[test]
fn vector_urlencoded_duplicate_keys_sorted() {
    let result = ash_core::ash_canonicalize_urlencoded("a=z&a=a&a=m").unwrap();
    assert_eq!(result, "a=a&a=m&a=z");
}
...
fn vector_binding_simple() {
    let result = ash_core::ash_normalize_binding("POST", "/api/test", "").unwrap();
...
fn vector_binding_with_query() {
    let result = ash_core::ash_normalize_binding("GET", "/api/users", "page=1&sort=name").unwrap();
    assert_eq!(result, "GET|/api/users|page=1&sort=name");
}

#[test]
fn vector_binding_query_sorted() {
    let result = ash_core::ash_normalize_binding("GET", "/api/users", "z=1&a=2").unwrap();
...
fn vector_binding_requires_leading_slash() {
    ...
    let result = ash_core::ash_normalize_binding("GET", "api/test", "");
...
fn vector_hash_body_format() {
    let result = ash_core::ash_hash_body(r#"{"amount":100,"recipient":"user123"}"#);
...
fn vector_hash_body_canonical_json() {
    let canonical = ash_core::ash_canonicalize_json(r#"{"amount":100,"recipient":"user123"}"#).unwrap();
    let hash = ash_core::ash_hash_body(&canonical);
...
fn vector_derive_client_secret_deterministic() {
    let secret1 = ash_core::ash_derive_client_secret(TEST_NONCE, TEST_CONTEXT_ID, TEST_BINDING).unwrap();
    let secret2 = ash_core::ash_derive_client_secret(TEST_NONCE, TEST_CONTEXT_ID, TEST_BINDING).unwrap();
    assert_eq!(secret1, secret2);
}

#[test]
fn vector_derive_client_secret_format() {
    let secret = ash_core::ash_derive_client_secret(TEST_NONCE, TEST_CONTEXT_ID, TEST_BINDING).unwrap();
...
fn vector_derive_client_secret_different_inputs() {
    let secret1 = ash_core::ash_derive_client_secret(TEST_NONCE, "ctx_a", TEST_BINDING).unwrap();
    let secret2 = ash_core::ash_derive_client_secret(TEST_NONCE, "ctx_b", TEST_BINDING).unwrap();
...
fn vector_derive_client_secret_different_nonces() {
    ...
    let secret1 = ash_core::ash_derive_client_secret(&nonce1, TEST_CONTEXT_ID, TEST_BINDING).unwrap();
    let secret2 = ash_core::ash_derive_client_secret(&nonce2, TEST_CONTEXT_ID, TEST_BINDING).unwrap();
    assert_ne!(secret1, secret2);
}

#[test]
fn vector_derive_client_secret_different_bindings() {
    let secret1 = ash_core::ash_derive_client_secret(TEST_NONCE, TEST_CONTEXT_ID, "POST|/api/a|").unwrap();
    let secret2 = ash_core::ash_derive_client_secret(TEST_NONCE, TEST_CONTEXT_ID, "POST|/api/b|").unwrap();
    assert_ne!(secret1, secret2);
}
...
fn vector_build_proof_v21_deterministic() {
    let client_secret = ash_core::ash_derive_client_secret(TEST_NONCE, TEST_CONTEXT_ID, TEST_BINDING).unwrap();
    let body_hash = ash_core::ash_hash_body(r#"{"amount":100}"#);

    let proof1 = ash_core::ash_build_proof(&client_secret, TEST_TIMESTAMP, TEST_BINDING, &body_hash).unwrap();
    let proof2 = ash_core::ash_build_proof(&client_secret, TEST_TIMESTAMP, TEST_BINDING, &body_hash).unwrap();
...
fn vector_build_proof_v21_format() {
    ...
    let proof = ash_core::ash_build_proof(&client_secret, TEST_TIMESTAMP, TEST_BINDING, &body_hash).unwrap();
...
fn vector_verify_proof_v21_valid() {
    ...
    let valid = ash_core::ash_verify_proof(TEST_NONCE, TEST_CONTEXT_ID, TEST_BINDING, TEST_TIMESTAMP, &body_hash, &proof).unwrap();
    assert!(valid);
}
...
fn vector_verify_proof_v21_invalid_proof() {
    ...
    let valid = ash_core::ash_verify_proof(TEST_NONCE, TEST_CONTEXT_ID, TEST_BINDING, TEST_TIMESTAMP, &body_hash, &wrong_proof).unwrap();
    assert!(!valid);
}
...
fn vector_verify_proof_v21_wrong_body() {
    let client_secret = ash_core::ash_derive_client_secret(TEST_NONCE, TEST_CONTEXT_ID, TEST_BINDING).unwrap();
    let body_hash1 = ash_core::ash_hash_body(r#"{"amount":100}"#);
    let body_hash2 = ash_core::ash_hash_body(r#"{"amount":200}"#);
    let proof = ash_core::ash_build_proof(&client_secret, TEST_TIMESTAMP, TEST_BINDING, &body_hash1).unwrap();

    let valid = ash_core::ash_verify_proof(TEST_NONCE, TEST_CONTEXT_ID, TEST_BINDING, TEST_TIMESTAMP, &body_hash2, &proof).unwrap();
    assert!(!valid);
}
...
fn vector_build_proof_unified_basic_no_scope_no_chain() {
    ...
    let result = ash_core::ash_build_proof_unified(&client_secret, TEST_TIMESTAMP, TEST_BINDING, payload, &[], None).unwrap();
...
fn vector_build_proof_unified_with_scope() {
    ...
    let result = ash_core::ash_build_proof_unified(&client_secret, TEST_TIMESTAMP, TEST_BINDING, payload, scope, None).unwrap();
...
fn vector_build_proof_unified_with_chain() {
    ...
    let client_secret = ash_core::ash_derive_client_secret(TEST_NONCE, TEST_CONTEXT_ID, binding).unwrap();
    let payload = r#"{"confirmed":true}"#;
    let previous_proof = "abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890";

    let result = ash_core::ash_build_proof_unified(&client_secret, TEST_TIMESTAMP, binding, payload, &[], Some(previous_proof)).unwrap();

    assert!(result.scope_hash.is_empty());
    assert!(!result.chain_hash.is_empty());
    assert_eq!(result.chain_hash, ash_core::ash_hash_proof(previous_proof).unwrap());
...
fn vector_timing_safe_compare_not_equal() {
    assert!(!ash_core::ash_timing_safe_equal(b"hello", b"world"));
    assert!(!ash_core::ash_timing_safe_equal(b"hello", b"hello!"));
    assert!(!ash_core::ash_timing_safe_equal(b"hello", b""));
}
...
fn fixed_vector_client_secret() {
    ...
    let secret = ash_core::ash_derive_client_secret(&nonce, context_id, binding).unwrap();

    assert_eq!(secret.len(), 64);
    let secret2 = ash_core::ash_derive_client_secret(&nonce, context_id, binding).unwrap();
...
fn vector_extract_scoped_fields_simple() {
    ...
    let result = ash_core::ash_extract_scoped_fields(&payload, scope).unwrap();
...
fn vector_full_workflow_unified_with_scope_and_chain() {
    ...
    let valid1 = ash_core::ash_verify_proof_unified(
    ...
    let result2 = ash_core::ash_build_proof_unified(&secret2, timestamp2, binding2, payload2, scope, Some(&result1.proof)).unwrap();

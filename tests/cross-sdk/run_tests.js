...
let ash;
try {
  ash = require('../../packages/ash-node/dist/index.js');
} catch (e) {
  console.log('Warning: Could not load compiled SDK, trying source...');
  try {
    // This would require ts-node or pre-compilation
    console.log('SDK not available. Build first: cd packages/ash-node && npm run build');
    process.exit(1);
  } catch (e2) {
    console.error('ERROR: ASH Node.js SDK not available');
    console.error('Build with: cd packages/ash-node && npm run build');
    process.exit(1);
  }
}

...
impl AshStore {
    ...
    fn create(&self, binding: &str, ttl_ms: u64) -> StoredContext {
        ...
        let context_id = format!("ash_{}", now);
        let nonce = format!("{:064x}", now);
...
async fn issue_context(
    ...
    let binding = normalize_binding("POST", endpoint, "").unwrap();
    let ctx = store.create(&binding, ttl_ms);

    let client_secret = derive_client_secret(&ctx.nonce, &ctx.context_id, &binding);
...
fn verify_ash(req: &HttpRequest, store: &AshStore, body: &str) -> Result<(), HttpResponse> {
    ...
    let binding = normalize_binding(req.method().as_str(), req.path(), req.query_string()).unwrap();
    let body_hash = hash_body(body);
    let client_secret = derive_client_secret(&stored.nonce, context_id, &binding);
    let expected_proof = build_proof_v21(&client_secret, timestamp, &binding, &body_hash);

    if !timing_safe_equal(proof, &expected_proof) {
        return Err(HttpResponse::Forbidden().json(ErrorResponse {

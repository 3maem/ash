...
```rust
use ash_core::{normalize_binding, build_proof_v21, hash_body};
use reqwest::Client;
...
async fn make_transfer() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let binding = normalize_binding("POST", "/api/transfer", "")?;
    let body_hash = hash_body(payload);
    let timestamp = current_time_ms().to_string();
    let proof = build_proof_v21(&ctx.client_secret, &timestamp, &binding, &body_hash);

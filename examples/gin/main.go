...
func AshMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Build binding
		binding := ash.NormalizeBinding(c.Request.Method, c.Request.URL.Path, c.Request.URL.RawQuery)
		...
		// Verify proof
		bodyHash := ash.HashBody(string(body))
		clientSecret := ash.DeriveClientSecret(stored.Nonce, contextID, binding)
		expectedProof := ash.BuildProofV21(clientSecret, timestamp, binding, bodyHash)
...
func issueContextHandler(c *gin.Context) {
	...
	binding := ash.NormalizeBinding("POST", req.Endpoint, "")
	ctx := ashStore.Create(binding, req.TtlMs)

	clientSecret := ash.DeriveClientSecret(ctx.Nonce, ctx.ContextID, binding)
...
func generateID() string {
	return fmt.Sprintf("ash_%d", time.Now().UnixNano())
}
